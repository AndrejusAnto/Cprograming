...
int main()
{
	...
	SDL_Color green = {0, 90, 0, 0};
	SDL_Color brown = {43, 29, 14, 0};
	...
	else
	{
		...
		else
		{
			...
			while (!quit)
			{
				...
				// printf("main bandymas\n");

				// primitives
				// SDL_SetRenderDrawColor(gRenderer, 0xFF, 0xFF, 0xFF, 0xFF);
				// SDL_RenderClear(gRenderer);
				// uzpildytas staciakampis
				void DrawRect(SDL_Renderer *r, int MinX, int MinY, int w, int h,
					SDL_Color c)
				{
					int Red = c.r;
					int Green = c.g;
				...
				for (int h = 0; h < hsize ; h++)
				{
					...
					for (int w = 0; w < wsize; w++)
					{
						if (tilemap[h][w] == 0)
						{
							DrawRect(gRenderer, w*sqw, h*sqh, sqw, sqh, green);
						}
						else
						{
							DrawRect(gRenderer, w*sqw, h*sqh, sqw, sqh, brown);
						}
						nh = nh + sqh;
					}
					nw = nw + sqw;
